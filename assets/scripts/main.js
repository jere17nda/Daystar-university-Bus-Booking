...
const cityEndpoint =
  "https://raw.githubusercontent.com/ASHWIN776/Countries-States-Cities-database/master/indiaCities.json";
...
function searchForms(evt) {
  ...
  if (evt.target.className.includes("searchInput")) {
    ...
    switch (id) {
      case "source_state":
        showSuggestions = showStates;
        break;

      case "source":
        showSuggestions = showCities;
        break;
...
function selectSuggestion(evt) {
  if (evt.target.nodeName === "LI") {
    ...
    if (
      this.previousElementSibling.id === "destination_state" ||
      this.previousElementSibling.id === "source_state"
    ) {
