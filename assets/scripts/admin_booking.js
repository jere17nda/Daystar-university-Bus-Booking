...
function showCustomerSuggestions(evt) {
  ...
  let suggestions = customerData
    .filter(({ customer_id }) => customer_id.match(regex))
    .map(({ customer_id }) => {
      const customerID = customer_id.replace(
        regex,
  ...
  if (!suggestions) suggestions = `<span>No Search Results Found</span>`;
...
function showRouteSuggestions(evt) {
  ...
  let suggestions = routeData
    .filter(({ route_cities }) => route_cities.match(regex))
    .map(
      ({
        route_id,
        route_cities,
        ...
        const viaCities = route_cities.replace(
          regex,
          `<span class="hl">${this.value.toUpperCase()}</span>`
        );
...
function showDestinationSuggestions(evt) {
  ...
  for (let i = 0; i < citiesArr.length; ++i)
    if (i > citiesArr.indexOf(sourceSelected)) suggestions.push(citiesArr[i]);

  console.log(suggestions);
  suggestions = suggestions
    .filter((city) => city.match(regex))
    .map((city) => {
      const cityName = city.replace(
        regex,
        `<span class="hl">${this.value.toUpperCase()}</span>`
...
function lockSuggestion(evt) {
  ...
  if (searchInput.id == "cid") {
    ...
    const findCustomer = customerData.find(
      ({ customer_id }) => customer_id === evt.target.innerText
    );
  ...
  if (
    ...
    if (!selected_id || evt.target.dataset.name === selected_id) {
      ...
      if (selected_id) {
        ...
        const citiesArr = convertToArray(
          document.querySelector("#routeSearch").dataset.id
        );

        console.log(sourceSelected, destSelected, citiesArr);
        const stepCost =
          document.querySelector("#routeSearch").dataset.stepcost;

        const amount =
          (citiesArr.indexOf(destSelected) -
            citiesArr.indexOf(sourceSelected)) *
          parseInt(stepCost);
        console.log(
          citiesArr.indexOf(destSelected),
          citiesArr.indexOf(sourceSelected),
