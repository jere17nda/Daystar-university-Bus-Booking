...
<body>
    ...
    if ($loggedIn && $_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST["submit"])) {
            ...
            $bus_exists = exist_buses($conn, $Model, $busno, $Capacity, $Driver, $Conductor, $Phone_Number);
            $bus_added = false;

            if (!$bus_exists) {
                // Route is unique, proceed

                $sql = "INSERT INTO `buses` (`Model`, `bus_no`, `bus_created`, `Capacity`, `Driver`, `Conductor`, `Phone_Number`) VALUES ('$Model', '$busno', current_timestamp(), '$Capacity', '$Driver', '$Conductor', '$Phone_Number')";
        ...
        if (isset($_POST["edit"])) {
            ...
            $id = $_POST["id"];
            $id_if_bus_exists = exist_buses($conn, $Model, $busno, $Capacity, $Driver, $Conductor, $Phone_Number);

            if (!$id_if_bus_exists || $id == $id_if_bus_exists) {
                $updateSql = "UPDATE `buses` SET `Model` = '$Model' WHERE `buses`.`id` = $id;";
                $updateSql = "UPDATE `buses` SET `bus_no` = '$busno' WHERE `buses`.`id` = $id;";
                $updateSql = "UPDATE `buses` SET `Capacity` = '$Capacity' WHERE `buses`.`id` = $id;";
                $updateSql = "UPDATE `buses` SET `Driver` = '$Driver' WHERE `buses`.`id` = $id;";
                $updateSql = "UPDATE `buses` SET `Conductor` = '$Conductor' WHERE `buses`.`id` = $id;";
                $updateSql = "UPDATE `buses` SET `Phone_Number` = '$Phone_Number' WHERE `buses`.`id` = $id;";
    ...
    <?php } else { ?>
        ...
        <section id="bus">
            ...
            <div id="bus-results">
                <div>
                    <button id="add-button" class="button btn-sm" type="button" data-bs-toggle="modal" data-bs-target="#addModal">ADD NEW BUS <i class="fas fa-plus"></i></button>
                </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="button edit-button " data-link="<?php echo $_SERVER['REQUEST_URI']; ?>" data-id="<?php
                                                                                                                                echo $id; ?>" data-busno="<?php echo $busno; ?>">Edit</button> <button class="button delete-button" data-bs-toggle="modal" data-bs-target="#deleteModal" data-id="<?php
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="addBusForm" action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="POST">
                        <div class="mb-3">
                            <label for="Model" class="form-label">Model</label><br>
                        ...
                        <div class="mb-3">
                            <label for="Capacity" class="form-label">Bus Capacity</label><br>
                        ...
                        <div class="mb-3">
                            <label for="Driver" class="form-label">Bus Driver</label><br>
                        ...
                        <div class="mb-3">
                            <label for="Conductor" class="form-label">Bus Conductor</label><br>
                        ...
                        <div class="mb-3">
                            <label for="Phone_Number" class="form-label">Contact</label><br>
